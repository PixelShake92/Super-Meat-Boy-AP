...
GOAL_BOSS_TOKENS = "boss_tokens"
GOAL_BANDAGES = "bandages"
GOAL_ACHIEVEMENTS = "achievements"

# Values match options.py in the APWorld — DO NOT reorder.
GOAL_INT_TO_STR = {
    0: GOAL_LARRIES,
    1: GOAL_LIGHT_WORLD,
    2: GOAL_DARK_WORLD,
...
WARP_ZONE_MAP = {
    ...
    4: [(0, 14, False, "The Key Master"), (1, 18, False, "The Fly Guy!"),
        (2, 8, False, "Brimstone"), (3, 7, True, "MMMMMM")],
...
# Dark warp host levels: lvl_type may read as warp type instead of TYPE_DARK.
DARK_WARP_HOST_LEVELS = {
    (w, host_lv)
    for w, zones in WARP_ZONE_MAP.items()
    for _, host_lv, is_dark, _ in zones
    if is_dark
}
...
class GameInterface:
    ...
    def read_warp_slots(self, sp, world):
        ...
        # W5 has split warp data: zone 1 (The Guy, slots 3-5) at 0x0EC4,
        # zones 0/2/3 at generic WORLD_BASES[5]+WARP_OFFSET=0x0C00
        if world == 5:
            generic_base = WORLD_BASES[5] + WARP_OFFSET
            if generic_base != warp_base:
                for i in range(NUM_WARP_SLOTS):
                    if i >= 3 and i <= 5:
                        continue
                    ca = generic_base + i * SLOT_SIZE + COMP_BYTE
                    try:
                        gval = self.pm.read_uchar(sp + ca)
                        if gval > 0 and vals[i] == 0:
                            vals[i] = gval
                    except: pass
        return vals
        ...
        if world == 5:
            ...
            if generic_base != warp_base:
                for i in range(NUM_WARP_SLOTS):
                    if i >= 3 and i <= 5:
                        continue
                    base_off = generic_base + i * SLOT_SIZE
                    try:
                        comp = self.pm.read_uchar(sp + base_off + COMP_BYTE)
                        tval = self.pm.read_float(sp + base_off + TIME_BYTE)
                        if comp > 0 and result[i][0] == 0:
                            result[i] = (comp, tval)
                    except: pass
        return result
    ...
    def game_flash(self, duration: float = 0.4) -> bool:
        ...
        flash_obj = *(*(base+0x30A1A0) + 0x20); +0xd00 = intensity, +0xd04 = secondary."""
        import struct as _struct, threading as _threading
        try:
            gsmb = self.pm.read_uint(self.base + 0x30A1A0)
            if not gsmb: return False
            flash_obj = self.pm.read_uint(gsmb + 0x20)
            if not flash_obj: return False
            intensity = _struct.pack('<f', 5.0)
            secondary = _struct.pack('<f', 1.0)
            end = time.time() + duration
            def _sustain():
                while time.time() < end:
                    try:
                        self.pm.write_bytes(flash_obj + 0xd00, intensity, 4)
                        self.pm.write_bytes(flash_obj + 0xd04, secondary, 4)
                    except: break
                    time.sleep(0.016)
            _threading.Thread(target=_sustain, daemon=True).start()
            return True
        except: return False

    def kill_player(self, world: int, level: int, is_dark: bool) -> bool:
        import struct as _struct, ctypes as _ctypes
        kernel32 = _ctypes.windll.kernel32
        try:
            lm = self.pm.read_uint(self.base + 0x30A46C)
            if not lm: return False
            level_name = f"{world}-{level}" + ("x" if is_dark else "")
            name_bytes = level_name.encode("ascii") + b"\x00"
            load_fn = self.base + 0x000AAB80
            handle = self.pm.process_handle
            mem = kernel32.VirtualAllocEx(handle, None, 112, 0x3000, 0x40)
            if not mem: return False
            name_addr, str_obj, sc_addr = mem, mem + 64, mem + 80
            self.pm.write_bytes(name_addr, name_bytes, len(name_bytes))
            self.pm.write_bytes(str_obj, _struct.pack("<IIII", name_addr, 0, 0, 0), 16)
            sc = bytearray()
            sc += b"\x68" + _struct.pack("<I", str_obj)
            sc += b"\xB9" + _struct.pack("<I", lm)
            sc += b"\xE8" + _struct.pack("<I", (load_fn - (sc_addr + len(sc) + 5)) & 0xFFFFFFFF)
            sc += b"\xC3"
            self.pm.write_bytes(sc_addr, bytes(sc), len(sc))
            t = kernel32.CreateRemoteThread(handle, None, 0, sc_addr, None, 0, None)
            if not t: kernel32.VirtualFreeEx(handle, mem, 0, 0x8000); return False
            kernel32.WaitForSingleObject(t, 3000)
            kernel32.CloseHandle(t)
            kernel32.VirtualFreeEx(handle, mem, 0, 0x8000)
            return True
        except: return False
...
class APClient:
    ...
    def __init__(self, log_callback, status_callback):
        ...
        self.achievement_token_count: int = 0
        ...
        self._pending_death_link: bool = False
        self._death_link_deaths: int = 0
        self._last_death_count: int = -1
        self._warp_exit_time: float = 0.0
        self._dl_skip_log_time: float = 0.0
        self.death_link_enabled: Optional[bool] = None
        self.on_death_link_state = None
        ...
        self._pending_aplus_recheck: Set[Tuple[int, int, str]] = set()
    ...
    def _compute_world_bitmask(self):
        ...
        APWorld rules (mirrors rules.py:363-371):
        - W6 requires Chapter 6 Key AND Meat Boy
        - W7 requires Chapter 7 Key AND Bandage Girl (always)
        - W7 *additionally* requires ALL enabled chapter keys when:
          goal is ch7 AND boss_tokens is disabled
          (when boss_tokens is enabled, tokens become the goal gate
          instead, so W7 access just needs Ch7 Key + Bandage Girl)
        """
        bitmask = 0
        goal = self._parse_goal(self.slot_data.get("goal", 0))
        chapters = self._chapters_set()
        ch6_enabled = "6" in chapters
        boss_tokens_on = self._parse_slot_bool(self.slot_data.get("boss_tokens", 0))
        for w in self.allowed_worlds:
            if w == 6:
                ...
                if (goal in (GOAL_LW_CHAPTER7, GOAL_DW_CHAPTER7)
                        and not boss_tokens_on):
                    required_chapters = [int(c) for c in chapters]
                    if not all(cw in self.allowed_worlds for cw in required_chapters):
                        continue
            bitmask |= (1 << (w - 1))
        return bitmask
    ...
    def _larries_accessible(self, boss_req_val):
        ...
        if tokens_enabled and goal == GOAL_LARRIES:
            token_req = self.slot_data.get("boss_token_req", 5)
            return self.boss_token_count >= token_req
        return True

    def _lw_drfetus_accessible(self):
        """Check lw_drfetus(): Chapter 6 Boss Key × lw_dr_fetus_req + token gating."""
        lw_req = self.slot_data.get("lw_dr_fetus_req", 5)
        if self.boss_key_counts.get(6, 0) < lw_req:
            return False
        goal = self._parse_goal(self.slot_data.get("goal", 0))
        tokens_enabled = self._parse_slot_bool(self.slot_data.get("boss_tokens", 0))
        ...
        for w in range(1, 7):
            ...
            if should_be_unlocked:
                ...
                if w <= 5:
                    ...
                    lw_keys = self.boss_key_counts.get(6, 0)
    ...
    def _dark_level_unlocked(self, world, level_1based):
        ...
        if not self._parse_slot_bool(self.slot_data.get("dark_world", 0)):
            return False
    ...
    def _grant_aplus_item(self, game, sp, world, level_1based):
        ...
        if real is not None and real > 0 and real <= par:
            ...
            if comp >= 0 and (comp & FLAG_COMPLETE):
                ...
                if current_t <= 0 or current_t > par:
    ...
    def _check_aplus_location(self, game, sp, world, level_0based, region, checks):
        ...
        if name:
            ...
            if loc and loc not in self.locations_checked:
                checks.append(loc)
                stored = game.read_time(sp, world, level_0based, region)
                if stored is not None and stored > par and time_val <= par:
                    game.write_time(sp, world, level_0based, region, time_val)
                    if region == "light":
                        self.real_best_times[(world, level_0based)] = time_val
        ...
        for zone_idx in range(4):
            ...
            if all_done:
                ...
                if info:
                    ...
                    self.log(f"[DEBUG] Warp all_done: loc_name={loc_name} loc_id={self.loc_id(loc_name) if loc_name else None}", "debug")
                    if loc_name:
                        loc = self.loc_id(loc_name)
                        if loc and loc not in self.locations_checked:
                            checks.append(loc)
                            self.log(f"Warp Complete: {loc_name}", "location")
                        elif loc in self.locations_checked:
                            self.log(f"[DEBUG] Warp already checked: {loc_name}", "debug")
    ...
    def _check_goal(self, goal, boss_tokens_enabled):
        ...
        APWorld completion conditions (rules.py):
        - larries/light_world/dark_world: state.has("Victory")
        - light_world_chapter7: Ch7 LW Level Key × 20 (+ Boss Tokens if enabled)
        - dark_world_chapter7: Ch7 DW Level Key × 20 (+ Boss Tokens if enabled)
        - boss_tokens: Boss Token × boss_tokens_amount  (tokens ALWAYS required)
        - bandages: Bandage × bandages_amount (+ Boss Tokens if enabled)
        - achievements: Achievement Token × achievement_tokens (+ Boss Tokens if enabled)
        """
        ...
        elif goal == GOAL_BOSS_TOKENS:
            return self._boss_tokens_sufficient(goal, True)
        ...
        elif goal == GOAL_ACHIEVEMENTS:
            tokens_needed = self.slot_data.get("achievement_tokens", 18)
            if self.achievement_token_count < tokens_needed:
                return False
            return self._boss_tokens_sufficient(goal, boss_tokens_enabled)
    ...
    def _boss_tokens_sufficient(self, goal, boss_tokens_enabled):
        ...
        chapters_raw = self.slot_data.get("chapters", set())
        if isinstance(chapters_raw, (list, set, tuple)):
            chapters = set(str(x) for x in chapters_raw)
        else:
            chapters = set()
        dw = self._parse_slot_bool(self.slot_data.get("dark_world", 0))

        needed = 0
        for i in range(1, 5):
            if str(i) in chapters:
                needed += 1
        if "5" in chapters and goal != GOAL_LARRIES:
            needed += 1
        if "6" in chapters and goal != GOAL_LIGHT_WORLD:
            needed += 1
        if "6" in chapters and dw and goal != GOAL_DARK_WORLD:
            needed += 1
    ...
    def _sweep_world(self, game, sp, world, dark_enabled,
                     warps_completed=None, bandages_enabled=False):
        ...
        if bandages_enabled and world <= 5:
            for zone_idx in range(4):
                info = WARP_ZONE_INFO.get((world, zone_idx))
                if not info:
                    continue
                ...
                for sub in range(3):
                    ...
                    if comp >= 0 and (comp & FLAG_BANDAGE):
                        bandage_num += 1
                        bname = name_fn(world, host_lv, bandage_num)
                        if bname:
                            loc = self.loc_id(bname)
                            if loc and loc not in self.locations_checked:
                                checks.append(loc)
                                self.log(f"Sweep: Warp Bandage: {bname}", "location")
                        game.write_comp(sp, world, si, "warp",
                                        comp & MASK_CLEAR_BANDAGE)
    ...
    async def _initial_sync_scan(self, game, sp, dark_enabled,
        ...
        bandage_count = 0
        bandages_enabled = self._parse_slot_bool(self.slot_data.get("bandages", 0))
        if bandages_enabled:
            for w in range(1, 6):
                for region in ("light", "dark"):
                    indices = range(20)
    ...
    async def set_death_link(self, enabled: bool):
        ...
        self.log(f"DeathLink {'ENABLED' if enabled else 'DISABLED'}", "info")
    ...
    async def handle_message(self, msg: dict):
        ...
        if cmd == "RoomInfo":
            ...
            test_names = [
                light_aplus_name(1, 1),
                dark_aplus_name(2, 8),
                light_aplus_name(6, 1),
            ]
            ...
            if self._parse_slot_bool(self.slot_data.get("dark_world", 0)):
                self.log("  Dark World: ENABLED", "info")
            if self._parse_slot_bool(self.slot_data.get("bandages", 0)):
                self.log("  Bandages: ENABLED", "info")
            if self._parse_slot_bool(self.slot_data.get("boss_tokens", 0)):
                self.log("  Boss Tokens: ENABLED", "info")
            boss_req = self.slot_data.get("boss_req", 17)
            self.log(f"  Boss Req: {boss_req}", "info")
            if self._parse_slot_bool(self.slot_data.get("death_link", 0)):
                amnesty = self.slot_data.get("death_link_amnesty", 10)
                self.log(f"  Death Link: ON (amnesty: {amnesty})", "info")
            chapters = self._chapters_set()
            if "6" not in chapters:
                self.log("  Chapter 6: DISABLED", "info")
            if "7" not in chapters:
                self.log("  Chapter 7: DISABLED", "info")

            self.set_status("Connected", "#00ff00")

            yaml_dl = self._parse_slot_bool(self.slot_data.get("death_link", 0))
            if self.death_link_enabled is None:
                self.death_link_enabled = yaml_dl
            await self.send_message({
                "cmd": "ConnectUpdate",
                "tags": ["DeathLink"] if self.death_link_enabled else [],
            })
            if self.death_link_enabled:
                self.log("  DeathLink: ENABLED", "info")
            else:
                self.log("  DeathLink: disabled", "info")
            if self.on_death_link_state:
                self.on_death_link_state(self.death_link_enabled)
            ...
            if "DeathLink" in tags:
                ...
                if source != self.slot and self.death_link_enabled:
                    self._pending_death_link = True
                    dl_msg = f"DeathLink from {source}"
                    if cause:
                        dl_msg += f": {cause}"
                    self.log(dl_msg, "error")
    ...
    def _process_item(self, item, game, sp):
        ...
        if iid in CHAPTER_KEY_ITEMS:
            ...
            if sp and self._parse_slot_bool(self.slot_data.get("dark_world", 0)):
        ...
        elif iid == ITEM_ACHIEVEMENT_TOKEN:
            self.achievement_token_count += 1
            needed = self.slot_data.get("achievement_tokens", 18)
            return f"Achievement Token ({self.achievement_token_count}/{needed})"
    ...
    async def game_monitor(self, game: GameInterface):
        ...
        warp_milestones_sent: Set[int] = set()
        speedrun_sent: Set[int] = set()
        deathless_sent: Set[Tuple[int, str]] = set()
        deathless_tracker: Dict = {}
        esc_cooldown = 0
        poll_count = 0
        ...
        goal = self._parse_goal(self.slot_data.get("goal", 0))
        dark_enabled = self._parse_slot_bool(self.slot_data.get("dark_world", 0))
        boss_req = self.slot_data.get("boss_req", 17)
        boss_tokens_enabled = self._parse_slot_bool(self.slot_data.get("boss_tokens", 0))
        bandages_enabled = self._parse_slot_bool(self.slot_data.get("bandages", 0))
        starting_chpt = self.slot_data.get("starting_chpt", 1)
        achievements_enabled = self._parse_slot_bool(self.slot_data.get("achievements", 0))
        deathless_enabled = self._parse_slot_bool(self.slot_data.get("deathless_achievements", 0))
        speedrun_enabled = self._parse_slot_bool(self.slot_data.get("speedrun_achievements", 0))
        if self.death_link_enabled is None:
            self.death_link_enabled = self._parse_slot_bool(self.slot_data.get("death_link", 0))
        death_link_amnesty = max(1, self.slot_data.get("death_link_amnesty", 10))
        chapters = self._chapters_set()
        ch6_enabled = "6" in chapters
        ch7_enabled = "7" in chapters
        ...
        self.log(f"Config: goal={goal}, dark={dark_enabled}, boss_req={boss_req}, "
                 ...
                 f"{', deathlink=ON (amnesty=' + str(death_link_amnesty) + ')' if self.death_link_enabled else ''}"
                 f"{', ch6=OFF' if not ch6_enabled else ''}"
                 f"{', ch7=OFF' if not ch7_enabled else ''}", "info")
        ...
        while self._running and game.connected:
            ...
            try:
                ...
                if is_warp(last_lvl_type) and not in_warp:
                    self._warp_exit_time = time.time()
                ...
                if world != last_world and last_world > 0 and world > 0:
                    if warp_entry_slots:
                        self.log(f"World change {last_world}->{world}: "
                                 f"clearing warp snapshots for W{list(warp_entry_slots.keys())}",
                                 "debug")
                    entry_comp.clear()
                    entry_region.clear()
                    entry_dual.clear()
                ...
                if playing == 1 and world <= 5 and world not in warp_entry_slots:
                    ...
                    self.log(f"Warp snapshot: W{world} lvl={level} type={lvl_type} "
                             f"slots={warp_entry_slots[world]}", "debug")
                ...
                if on_level_select:
                    ...
                    if just_arrived or pending_recheck or poll_count % 500 == 499:
                        ...
                        sweep_checks = self._sweep_world(
                            game, sp, world, dark_enabled,
                            warps_completed, bandages_enabled)
                        if sweep_checks:
                ...
                if self.death_link_enabled and playing == 1:
                    ...
                    # Without an entry baseline, dual_region can never resolve.
                    if (self._pending_death_link
                            and 0 <= actual_lvl < 20 and sp
                            and (world, actual_lvl) not in entry_dual):
                        _lc = game.read_comp(sp, world, actual_lvl, "light")
                        _dc = game.read_comp(sp, world, actual_lvl, "dark")
                        if _lc >= 0 and _dc >= 0:
                            entry_dual[(world, actual_lvl)] = (_lc, _dc)
                            self.log(
                                f"[DeathLink] lazy entry_dual snapshot for "
                                f"W{world} lv{actual_lvl}: light={_lc:#04x} "
                                f"dark={_dc:#04x}",
                                "debug")

                    dual_region = None
                    if 0 <= actual_lvl < 20 and sp:
                        _dual = entry_dual.get((world, actual_lvl))
                        if _dual:
                            _elc, _edc = _dual
                            _clc = game.read_comp(sp, world, actual_lvl, "light")
                            _cdc = game.read_comp(sp, world, actual_lvl, "dark")
                            if _cdc >= 0 and _edc >= 0 and _cdc != _edc:
                                dual_region = "dark"
                            elif _clc >= 0 and _elc >= 0 and _clc != _elc:
                                dual_region = "light"

                    # sp+0x38C0 (0 = light variant active, 1 = dark variant
                    region_signal = None
                    if sp:
                        try:
                            dark_flag = game.pm.read_uchar(sp + 0x38C0)
                            if dark_flag == 1:
                                region_signal = "dark"
                            elif dark_flag == 0:
                                region_signal = "light"
                            ...
                            pass
                    if region_signal is None:
                        if dual_region in ("light", "dark"):
                            region_signal = dual_region
                        elif last_detected_region in ("light", "dark"):
                            region_signal = last_detected_region

                    structural_ok = (0 <= level < 20 and 1 <= world <= 7)
                    cooldown_ok = (time.time() - self._warp_exit_time) > 1.5
                    has_region = region_signal is not None
                    not_in_warp = not in_warp

                    if (self._pending_death_link
                            and structural_ok and cooldown_ok and has_region
                            and not_in_warp):
                        is_dark = (region_signal == "dark")
                        self._pending_death_link = False
                        flashed = game.game_flash()
                        await asyncio.sleep(0.35)
                        ok = game.kill_player(world, lv1, is_dark)
                        variant = "dark" if is_dark else "light"
                        self.log(
                            f"Killed by DeathLink! ({'OK' if ok else 'FAILED'}) [{variant}]",
                            "error")
                        if ok and 0 <= actual_lvl < 20 and sp:
                            await asyncio.sleep(0.20)
                            sp_post = game.get_sp()
                            if sp_post:
                                _new_lc = game.read_comp(sp_post, world, actual_lvl, "light")
                                _new_dc = game.read_comp(sp_post, world, actual_lvl, "dark")
                                if _new_lc >= 0 and _new_dc >= 0:
                                    entry_dual[(world, actual_lvl)] = (_new_lc, _new_dc)
                    elif self._pending_death_link:
                        _now = time.time()
                        if _now - self._dl_skip_log_time > 2.0:
                            self._dl_skip_log_time = _now
                            warp_age = _now - self._warp_exit_time
                            reasons = []
                            if not structural_ok:
                                reasons.append(f"structural (w={world} lv={level})")
                            if not cooldown_ok:
                                reasons.append(f"warp_cooldown ({warp_age:.1f}s)")
                            if not has_region:
                                reasons.append("no region signal yet")
                            reason_str = ", ".join(reasons) or "unknown"
                            self.log(
                                f"[DeathLink] pending: {reason_str} "
                                f"| w={world} lv={level} dual={dual_region} "
                                f"last={last_detected_region}",
                                "debug")

                    # Track deaths using save-data counter (sp+0x38AC)
                    current_dc = game.read_death_count(sp)
                    if current_dc >= 0:
                        if self._last_death_count >= 0 and current_dc > self._last_death_count:
                            new_deaths = current_dc - self._last_death_count
                            self._death_link_deaths += new_deaths
                            if self._death_link_deaths >= death_link_amnesty:
                                self._death_link_deaths = 0
                                await self.send_message({
                                    "cmd": "Bounce",
                                    "tags": ["DeathLink"],
                                    "data": {
                                        "time": time.time(),
                                        "source": self.slot,
                                        "cause": f"{self.slot} died in Super Meat Boy",
                                    },
                                })
                                self.log(f"DeathLink sent! ({death_link_amnesty} deaths)", "error")
                            else:
                                self.log(f"[DeathLink] {self._death_link_deaths}/{death_link_amnesty} deaths", "game")
                        self._last_death_count = current_dc
                ...
                if beaten == 1 and last_beaten == 0 and actual_level >= 0:
                    self.log(f"Beaten flag fired: W{world} lvl={actual_level} "
                             f"type={lvl_type}", "debug")
                    if actual_level < 20 and 1 <= world <= 7:
                        lv1 = actual_level + 1
                        is_dark_host = (world, lv1) in DARK_WARP_HOST_LEVELS
                        if lvl_type == TYPE_LIGHT:
                            cname = light_completion_name(world, lv1)
                        elif lvl_type == TYPE_DARK or (is_warp(lvl_type) and is_dark_host):
                            cname = dark_completion_name(world, lv1)
                        else:
                            cname = None
                        if cname:
                            loc = self.loc_id(cname)
                            self.log(f"[DEBUG] Beaten: cname='{cname}' loc={loc} checked={loc in self.locations_checked if loc else 'N/A'}", "debug")
                            if loc and loc not in self.locations_checked:
                                await self.send_location_checks([loc])
                                self.log(f"Beaten flag: {cname}", "location")
                        effective_region = "dark" if (lvl_type == TYPE_DARK or (is_warp(lvl_type) and is_dark_host)) else type_to_region(lvl_type)
                        if effective_region in ("light", "dark"):
                            comp = game.read_comp(sp, world, actual_level, effective_region)
                            if comp >= 0 and not (comp & FLAG_COMPLETE):
                                game.write_comp(sp, world, actual_level, effective_region,
                                                comp | FLAG_COMPLETE)
                                self.log(f"Force-wrote completion for W{world} "
                                         f"{lv1}{('X' if effective_region == 'dark' else '')}",
                                         "debug")
                            self._pending_aplus_recheck.add((world, actual_level, effective_region))
                    ...
                    if sp2:
                        ...
                        if last_world in warp_entry_slots and last_world <= 5:
                            ...
                            if entry_slots != current_slots:
                                ...
                                self.log(f"Warp mid-play: W{last_world} lvl {last_level}->{level}", "debug")
                                self.log(f"  entry={entry_slots}", "debug")
                                self.log(f"  curr ={current_slots}", "debug")
                                if bandages_enabled:
                        ...
                        if last_level in (11, 13) and last_world == 5 or (last_level == 13 and last_world == 4):
                            self.log(f"[DEBUG] Warp host exit: W{last_world} lvl={last_level} dual={dual is not None} key={old_key}", "debug")
                        if dual:
                            el, ed = dual
                ...
                if playing == 0 and last_playing == 1:
                    ...
                    if sp2 and last_world > 0:
                        ...
                        if last_world in warp_entry_slots and last_world <= 5:
                            ...
                            if entry_slots != current_slots:
                                warp_changed = True
                                self.log(f"Warp exit: W{last_world} lvl={exit_level}", "debug")
                                self.log(f"  entry={entry_slots}", "debug")
                                self.log(f"  curr ={current_slots}", "debug")
                                if bandages_enabled:
                                    self._check_warp_bandages(
                        ...
                        if exit_level >= 0:
                            ...
                            effective_exit_region = exit_region_detected
                            if not effective_exit_region and dark_enabled and exit_level >= 0:
                                if (last_world, exit_level + 1) in DARK_WARP_HOST_LEVELS:
                                    effective_exit_region = "dark"
                            if effective_exit_region:
                                self._check_aplus_location(
                                    game, sp2, last_world, exit_level, effective_exit_region, checks)
                            else:
                                self._check_aplus_location(
                        ...
                        if 1 <= last_world <= 7:
                            sweep_checks = self._sweep_world(
                                game, sp2, last_world, dark_enabled,
                                warps_completed, bandages_enabled)
                            if sweep_checks:
                                await self.send_location_checks(sweep_checks)
...
class SMBClientApp:
    def __init__(self):
        ...
        self.death_link_var = tk.BooleanVar(value=False)
    ...
    def create_widgets(self):
        ...
        self.debug_check = ttk.Checkbutton(
            btn_frame, text="Debug log",
            variable=self.debug_logging_var,
            command=self.save_config)
        self.debug_check.pack(side=tk.RIGHT, padx=(0, 10))
        self.death_link_check = ttk.Checkbutton(
            btn_frame, text="DeathLink",
            variable=self.death_link_var,
            command=self.on_toggle_death_link)
        self.death_link_check.pack(side=tk.RIGHT, padx=(0, 10))
    ...
    def log(self, message: str, tag: str = "info"):
        if tag == "debug" and not self.debug_logging_var.get():
            return
        def _log():
            ...
            if client.loop and client.loop.is_running():
                try:
                    asyncio.run_coroutine_threadsafe(
                        client.set_death_link(enabled), client.loop)
                except Exception as e:
                    self.log(f"DeathLink toggle error: {e}", "error")
            else:
                client.death_link_enabled = enabled
                self.log(f"DeathLink {'ENABLED' if enabled else 'DISABLED'} "
                         f"(applies on connect)", "info")
        else:
            self.log(f"DeathLink {'ENABLED' if enabled else 'DISABLED'} "
                     f"(applies on connect)", "info")

    def _sync_death_link_check(self, enabled: bool):
        """Called from the client (via connect handshake) to reflect the
        effective DeathLink state on the button. Marshalled onto the Tk thread."""
        try:
            self.root.after(0, lambda: self.death_link_var.set(bool(enabled)))
        except: pass

    def load_settings(self):
        self.server_var.set(self.config.get("server", "archipelago.gg:38281"))
        self.slot_var.set(self.config.get("slot", ""))
        self.debug_logging_var.set(bool(self.config.get("debug_logging", False)))
        self.death_link_var.set(bool(self.config.get("death_link", False)))
    ...
    def connect(self):
        ...
        self.client.on_death_link_state = self._sync_death_link_check
        if "death_link" in self.config:
            self.client.death_link_enabled = self.death_link_var.get()
